import { GL_DATA_UNSIGNED_BYTE, GL_DATA_UNSIGNED_INT, GL_DEPTH_COMPONENT, GL_DEPTH_COMPONENT16, GL_LINEAR, GL_NEAREST_MIPMAP_LINEAR, GL_PIXEL_UNSIGNED_BYTE, GL_REPEAT, GL_RGBA, GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_TEXTURE_MIN_FILTER, GL_TEXTURE_WRAP_S, GL_TEXTURE_WRAP_T, } from "./webgl.js";
export function fetch_image(path) {
...
export function create_texture_from(gl, image) {
...
export function create_texture_rgba(gl, width, height) {
    let texture = gl.createTexture();
    gl.bindTexture(GL_TEXTURE_2D, texture);
...
export function create_render_buffer(gl, width, height) {
...
export function create_texture_depth(gl, width, height) {
    let texture = gl.createTexture();
    gl.bindTexture(GL_TEXTURE_2D, texture);
    gl.texImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT16, width, height, 0, GL_DEPTH_COMPONENT, GL_DATA_UNSIGNED_INT, null);
    gl.texParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    gl.texParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    return texture;
}
